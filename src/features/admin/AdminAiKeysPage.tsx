    ...
    <AppChrome section="operations" breadcrumbs={crumbs} account={account}>
      <div data-myriale-theme="archive" className="min-h-[calc(100vh-118px)] bg-[image:var(--myr-screen-background)] p-3 font-myr-body text-myr-ink md:p-5">
        <main className="mx-auto grid min-h-[calc(100vh-158px)] max-w-[1180px] content-start rounded-myr-panel border border-white/40 bg-[image:var(--myr-paper-background)] [background-size:26px_100%,auto] p-5 shadow-myr-panel md:p-8" aria-label="AI Provider管理">
          <p className="mb-2 text-[0.6875rem] font-extrabold tracking-[0.16em] text-[#6d587a] uppercase">Operations / AI providers</p>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
            <section className="rounded-myr-card border border-myr-ink/15 bg-myr-paper/75 p-5 shadow-myr-card" aria-labelledby="provider-registration-heading">
              <p className="mb-2 font-myr-mono text-[0.6875rem] font-black tracking-[0.14em] text-myr-ruby uppercase">Credential override</p>
              <h2 id="provider-registration-heading" className="m-0 font-myr-display text-[clamp(1.75rem,3vw,2.5rem)] leading-none tracking-[-0.04em]">管理画面からキーを登録</h2>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <label className="grid gap-2 text-xs font-black tracking-[0.04em] text-myr-slate">Provider
                  <select className="!rounded-myr-card !border !border-myr-ink/15 !bg-[#fffef9] !px-3 !py-3 !text-base !text-myr-ink" value={provider} onChange={(event) => changeProvider(event.target.value)}>
                    <option value="runpod">Runpod</option>
                    <option value="openai">OpenAI</option>
                  </select>
                </label>
                <label className="grid gap-2 text-xs font-black tracking-[0.04em] text-myr-slate">表示名
                  <input aria-label="表示名" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
                </label>
              </div>
              <label className="mt-4 grid gap-2 text-xs font-black tracking-[0.04em] text-myr-slate">APIキー
            ...
            <aside className="rounded-myr-card border border-myr-ink/15 bg-myr-vellum/45 p-5" aria-label="設定の優先順位">
              <p className="mb-2 font-myr-mono text-[0.6875rem] font-black tracking-[0.14em] text-myr-ruby uppercase">Resolution order</p>
              <h2 className="m-0 font-myr-display text-[clamp(1.75rem,3vw,2.5rem)] leading-none tracking-[-0.04em]">設定の優先順位</h2>
          ...
          <section className="mt-4 overflow-hidden rounded-myr-card border border-myr-ink/15 bg-myr-paper/80 shadow-myr-card" aria-label="AIキー一覧">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] border-collapse text-left text-sm">
                <thead className="bg-myr-iris/10 font-myr-mono text-[0.6875rem] tracking-[0.1em] text-myr-slate uppercase"><tr><th className="px-4 py-3">Provider</th><th className="px-4 py-3">接続設定</th><th className="px-4 py-3">キー</th><th className="px-4 py-3">検証状態</th><th className="px-4 py-3">操作</th></tr></thead>
                <tbody>{keys.map((key) => (
                  <tr className="border-t border-myr-ink/10" key={key.provider} data-testid={`ai-key-row-${key.provider}`}>
                    ...
                    <td className="px-4 py-4"><div className="flex flex-wrap gap-1.5">{key.active && <span className="rounded-full bg-myr-ink px-2.5 py-1 text-[0.6875rem] font-black text-myr-paper">使用中</span>}<span className={`rounded-full px-2.5 py-1 text-[0.6875rem] font-black ${key.credentialSource === 'environment' ? 'bg-myr-iris/15 text-myr-iris' : key.credentialSource === 'database' ? 'bg-myr-gold/30 text-myr-ink' : 'bg-myr-ink/8 text-myr-slate'}`}>{sourceLabel(key.credentialSource)}</span></div></td>
