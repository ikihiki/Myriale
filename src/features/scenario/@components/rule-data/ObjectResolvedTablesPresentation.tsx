...
export function ObjectResolvedTablesPresentation({ value, object, states, actions, rules, conflicts, onChange, onNotice }: Props) {
  ...
  return <section className="grid gap-5" aria-label="Object resolved configuration">
    ...
    <section className="grid gap-2" aria-label="状態">
      ...
      <div className="overflow-x-auto rounded-xl border border-[#17151f]/12 bg-white/70"><table aria-label="Object states" className={tableClass}><thead><tr><th className={cellClass}>詳細</th><th className={cellClass}>表示名</th><th className={cellClass}>stable code</th><th className={cellClass}>型</th><th className={cellClass}>公開</th><th className={cellClass}>更新</th><th className={cellClass}>初期値</th></tr></thead><tbody>
        {states.map((row) => <tr key={row.code} aria-invalid={Boolean(row.conflict)}><td className={cellClass}><Button size="sm" variant="secondary" disabled={Boolean(row.conflict)} aria-label={`${row.code}の状態を確認`} onClick={() => setSelection({ kind: 'state', key: row.code })}>詳細</Button></td><td className={cellClass}><strong>{row.label}</strong>{row.conflict && <span className="block text-xs text-[#a8324a]">競合</span>}</td><td className={`${cellClass} font-mono text-xs`}>{row.code}</td><td className={cellClass}>{row.valueType}</td><td className={cellClass}>{row.visibility}</td><td className={cellClass}>{row.updateAuthority ?? 'rules'}</td><td className={cellClass}>{row.updateAuthority === 'ai' ? 'interaction時に生成' : row.effectiveInitialValue}</td></tr>)}
    ...
    <EditPane layer={1} open={Boolean(selectedState)} onOpenChange={(open) => { if (!open) setSelection(null); }} eyebrow="状態" title={selectedState?.label ?? '状態'} description={selectedState?.inherited ? 'このObjectでは初期値のみ変更できます。項目の定義はObject種類側で変更します。' : 'このObjectだけで使う状態の契約を編集します。'} footer={<Button onClick={() => setSelection(null)}>閉じる</Button>}>
      {selectedState && (selectedState.inherited ? <div className={editorClass}>
        <ReadOnlyValue label="stable code" value={selectedState.code} /><ReadOnlyValue label="表示名" value={selectedState.label} /><ReadOnlyValue label="型" value={selectedState.valueType} /><ReadOnlyValue label="公開" value={selectedState.visibility} /><ReadOnlyValue label="更新権限" value={selectedState.updateAuthority ?? 'rules'} />{selectedState.aiGuidance && <ReadOnlyValue label="AI guidance" value={selectedState.aiGuidance ?? ''} />}{(selectedState.updateAuthority ?? 'rules') === 'rules' && <ReadOnlyValue label="基本の初期値" value={selectedState.baseInitialValue} />}
        {(selectedState.updateAuthority ?? 'rules') === 'rules' && <><label>このObjectの初期値<Input aria-label={`${selectedState.code}の初期値`} value={selectedState.hasInitialOverride ? selectedState.effectiveInitialValue : ''} placeholder={selectedState.baseInitialValue} onChange={(event) => setInitialOverride(selectedState, event.target.value)} /></label><Button size="sm" variant="text" disabled={!selectedState.hasInitialOverride} onClick={() => setInitialOverride(selectedState, '')}>継承値へ戻す</Button></>}
        ...
        <MyrialeSelect label="更新権限" value={selectedState.updateAuthority} onValueChange={(updateAuthority) => replaceLocalState(selectedState, { updateAuthority: updateAuthority as ScenarioStateField['updateAuthority'], ...(updateAuthority === 'ai' ? { defaultValue: '' } : {}) })} options={[{ value: 'rules', label: 'rules' }, { value: 'ai', label: 'ai' }]} />
        {selectedState.updateAuthority === 'rules' ? <label>default<Input aria-label="Object state default" value={selectedState.defaultValue} onChange={(event) => replaceLocalState(selectedState, { defaultValue: event.target.value })} /></label> : <label>AI guidance<Textarea aria-label="Object state AI guidance" value={selectedState.aiGuidance} onChange={(event) => replaceLocalState(selectedState, { aiGuidance: event.target.value })} /></label>}
