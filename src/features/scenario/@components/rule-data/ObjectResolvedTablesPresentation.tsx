...
export function ObjectResolvedTablesPresentation({ value, object, states, actions, rules, conflicts, onChange, onNotice }: Props) {
  ...
  const replaceLocalState = (row: ResolvedStateField, patch: Partial<ScenarioStateField>) => {
    ...
    onChange({
      ...
      actions: object.actions.map((action) => action.availabilityStateCode === previousCode ? { ...action, availabilityStateCode: nextState.code } : action),
    });
  ...
  return <section className="grid gap-5" aria-label="Object resolved configuration">
    ...
    <EditPane layer={1} open={Boolean(selectedAction)} onOpenChange={(open) => { if (!open) setSelection(null); }} eyebrow="アクション" title={selectedAction?.label ?? 'アクション'} description={selectedAction?.inherited ? 'この項目の定義はObject種類側で変更します。' : 'このObjectだけで使うアクション契約を編集します。'} footer={<Button onClick={() => setSelection(null)}>閉じる</Button>}>
      {selectedAction && (selectedAction.inherited ? <div className={editorClass}>
        <ReadOnlyValue label="stable code" value={selectedAction.code} /><ReadOnlyValue label="表示名" value={selectedAction.label} /><ReadOnlyValue label="説明" value={selectedAction.description} /><ReadOnlyValue label="visibility" value={selectedAction.visibility} /><ReadOnlyValue label="提示条件" value={selectedAction.availability === 'always' ? '常に提示' : selectedAction.availabilityStateCode} /><ReadOnlyValue label="引数" value={selectedAction.argumentFields.map((item) => `${item.label} / ${item.code}`).join(', ') || 'なし'} />
      </div> : <div className={editorClass}>
        ...
        <MyrialeSelect label="提示条件" value={selectedAction.availability} onValueChange={(availability) => replaceLocalAction(selectedAction, { availability: availability as ScenarioTypeAction['availability'] })} options={[{ value: 'always', label: '常に提示' }, { value: 'state-equals', label: '状態がdefaultと一致' }]} />
        {selectedAction.availability === 'state-equals' && <MyrialeSelect label="提示条件に使う状態" value={selectedAction.availabilityStateCode} onValueChange={(availabilityStateCode) => replaceLocalAction(selectedAction, { availabilityStateCode })} options={states.filter((item) => !item.conflict).map((item) => ({ value: item.code, label: `${item.label} / ${item.code}` }))} />}
