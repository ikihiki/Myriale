...
export function RuleConfigurationEditorPresentation({ label, stateFields, actions, onChange, onRenameState, onRenameAction, onDeleteState, onDeleteAction }: Props) {
  ...
  return <section aria-label={`${label} rule configuration`} className="grid gap-5 rounded-xl border border-[#17151f]/12 bg-[#fffef9]/80 p-3">
    <header><strong>{label}</strong><p className="text-xs text-myr-ink-subtle">状態とAction契約を一覧から選び、重ねた編集ペインで詳しく編集します。</p></header>
    <section className="grid gap-2" aria-labelledby={`${label}-states-heading`}>
      <div className="flex items-center justify-between gap-2"><strong id={`${label}-states-heading`} className="text-sm">状態 fields</strong><Button size="sm" variant="secondary" onClick={() => { const index = stateFields.length; onChange({ stateFields: [...stateFields, createStateField()], actions }); setEditing({ kind: 'state', index }); }}>{label}へ状態を追加</Button></div>
      ...
      </tbody></table>{stateFields.length === 0 && <p className="p-4 text-xs text-myr-ink-subtle">local stateはありません。</p>}</div>
    </section>
    <section className="grid gap-2" aria-labelledby={`${label}-actions-heading`}>
      <div className="flex items-center justify-between gap-2"><strong id={`${label}-actions-heading`} className="text-sm">Actions</strong><Button size="sm" variant="secondary" onClick={() => { const index = actions.length; onChange({ stateFields, actions: [...actions, createTypeAction()] }); setEditing({ kind: 'action', index }); }}>{label}へActionを追加</Button></div>
      ...
      </tbody></table>{actions.length === 0 && <p className="p-4 text-xs text-myr-ink-subtle">local actionはありません。</p>}</div>
    </section>

    <EditPane layer={1} open={Boolean(state)} onOpenChange={(open) => { if (!open) setEditing(null); }} eyebrow={label} title={state?.label ?? '状態を編集'} description="stable code、型、default、公開範囲を編集します。" footer={<Button onClick={() => setEditing(null)}>状態の編集を完了</Button>}>
      {state && editing?.kind === 'state' && <div className={editorClass}>
        <label>stable code<Input aria-label={`${label} state code ${editing.index + 1}`} value={state.code} onChange={(event) => onRenameState ? onRenameState(editing.index, event.target.value) : replaceState(editing.index, { code: event.target.value })} /></label>
        <label>表示名<Input aria-label={`${label} state label ${editing.index + 1}`} value={state.label} onChange={(event) => replaceState(editing.index, { label: event.target.value })} /></label>
        ...
        <label>default<Input aria-label={`${label} state default ${editing.index + 1}`} value={state.defaultValue} onChange={(event) => replaceState(editing.index, { defaultValue: event.target.value })} /></label>
    ...
    <EditPane layer={1} open={editing?.kind === 'action' && Boolean(action)} onOpenChange={(open) => { if (!open) setEditing(null); }} eyebrow={label} title={action?.label ?? 'Actionを編集'} description="Action契約と引数を編集します。実行ruleは別のrule tableで管理します。" footer={<Button onClick={() => setEditing(null)}>アクションの編集を完了</Button>}>
      {action && editing?.kind === 'action' && <div className={editorClass}>
        <label>stable code<Input aria-label={`${label} action code ${editing.index + 1}`} value={action.code} onChange={(event) => onRenameAction ? onRenameAction(editing.index, event.target.value) : replaceAction(editing.index, { code: event.target.value })} /></label>
    ...
    <EditPane layer={2} open={editing?.kind === 'argument' && Boolean(argument)} onOpenChange={(open) => { if (!open) setEditing(action ? { kind: 'action', index: actionIndex } : null); }} eyebrow="Action引数" title={argument?.label ?? '引数を編集'} description="structured action argumentの契約を編集します。" footer={<Button onClick={() => setEditing({ kind: 'action', index: actionIndex })}>引数の編集を完了</Button>}>
      {action && argument && editing?.kind === 'argument' && <div className={editorClass}>
        <label>stable code<Input aria-label={`${label} action ${actionIndex + 1} argument code ${editing.index + 1}`} value={argument.code} onChange={(event) => replaceAction(actionIndex, { argumentFields: action.argumentFields.map((field, index) => index === editing.index ? { ...field, code: event.target.value } : field) })} /></label>
        <label>表示名<Input aria-label={`${label} action ${actionIndex + 1} argument label ${editing.index + 1}`} value={argument.label} onChange={(event) => replaceAction(actionIndex, { argumentFields: action.argumentFields.map((field, index) => index === editing.index ? { ...field, label: event.target.value } : field) })} /></label>
