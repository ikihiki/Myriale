  ...
  return <>
    ...
    <EditPane layer={1} open={Boolean(state)} onOpenChange={(open) => { if (!open) setEditing(null); }} eyebrow="状態" title={state?.label ?? '状態を編集'} description="stable code、型、default、公開範囲を編集します。" footer={<Button onClick={() => setEditing(null)}>状態の編集を完了</Button>}>
      {state && editing?.kind === 'state' && <div className={editorClass}>
        ...
        <MyrialeSelect label="更新権限" value={state.updateAuthority ?? 'rules'} onValueChange={(updateAuthority) => replaceState(editing.index, { updateAuthority: updateAuthority as ScenarioStateField['updateAuthority'], ...(updateAuthority === 'ai' ? { defaultValue: '' } : {}) })} options={[{ value: 'rules', label: 'rules' }, { value: 'ai', label: 'ai' }]} />
        {(state.updateAuthority ?? 'rules') === 'rules' ? <label>default<Input aria-label={`状態${editing.index + 1}のdefault`} value={state.defaultValue} onChange={(event) => replaceState(editing.index, { defaultValue: event.target.value })} /></label> : <label>AI guidance<Textarea aria-label={`状態${editing.index + 1}のAI guidance`} value={state.aiGuidance ?? ''} onChange={(event) => replaceState(editing.index, { aiGuidance: event.target.value })} /></label>}
