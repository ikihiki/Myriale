...
export function LocationsObjectsEditorPresentation({ value, onChange, onNotice }: Props) {
  ...
  return <section aria-label="場所とオブジェクト" className="grid gap-7">
    ...
    <EditPane open={Boolean(object)} onOpenChange={(open) => { if (!open) { setEditing(null); setEditingOperationIndex(null); } }} eyebrow="オブジェクト" title={object?.name ?? 'オブジェクトを編集'} description="Typeの継承結果とObject local差分を編集します。" footer={<Button onClick={() => setEditing(null)}>編集を完了</Button>}>{object && <div className={editorClass}>
      ...
      <section aria-label="ordered Type mixins" className="grid gap-2 rounded-xl border border-[#17151f]/12 p-3"><strong>ordered Type mixins</strong>{object.mixinTypeCodes.map((code, index, codes) => <div key={`${code}-${index}`} className="flex items-center gap-2"><span className="flex-1">{value.objectTypes.find((type) => type.code === code)?.name ?? code}</span><Button size="sm" variant="text" disabled={index === 0} onClick={() => { const next = [...codes]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; replaceObject({ ...object, mixinTypeCodes: next }); }}>↑</Button><Button size="sm" variant="text" disabled={index === codes.length - 1} onClick={() => { const next = [...codes]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; replaceObject({ ...object, mixinTypeCodes: next }); }}>↓</Button><Button size="sm" variant="text" onClick={() => replaceObject({ ...object, mixinTypeCodes: codes.filter((_, itemIndex) => itemIndex !== index) })}>削除</Button></div>)}<MyrialeSelect label="Type mixinを追加" value="" onValueChange={(code) => { if (!object.mixinTypeCodes.includes(code)) replaceObject({ ...object, mixinTypeCodes: [...object.mixinTypeCodes, code] }); }} options={value.objectTypes.filter((type) => !object.mixinTypeCodes.includes(type.code)).map((type) => ({ value: type.code, label: `${type.name} / ${type.code}` }))} /></section>
