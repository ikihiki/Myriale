  ...
  return (
    <section aria-label="場所とオブジェクト" className="grid gap-7">
      ...
      <section className="grid gap-4" aria-labelledby="objects-heading">
        <header className="flex items-end justify-between gap-4 max-md:items-start"><div><h2 id="objects-heading">オブジェクト</h2><p>種類を選び、初期配置先と状態の上書きを設定します。</p></div><Button size="sm" variant="secondary" onClick={addObject}>オブジェクトを追加</Button></header>
        <div className="overflow-x-auto rounded-2xl border border-[#17151f]/15 bg-white/55 shadow-[0_12px_30px_rgba(23,21,31,.07)]">
          <table className={tableClass}><thead className="bg-[#17151f]/[.045] text-xs text-myr-slate-muted"><tr><th className={cellClass}>編集</th><th className={cellClass}>表示名</th><th className={cellClass}>stable code</th><th className={cellClass}>種類</th><th className={cellClass}>初期配置</th></tr></thead><tbody>
            {value.objects.map((item) => { const mixins = item.mixinTypeCodes.map((code) => value.objectTypes.find((candidate) => candidate.code === code)?.name ?? code); const place = value.locations.find((candidate) => candidate.code === item.initialLocationCode); return <tr key={item.code} className="hover:bg-white/65"><td className={cellClass}><Button size="sm" variant="secondary" onClick={() => setEditing({ kind: 'object', code: item.code })} aria-label={`${item.name}を編集`}>編集</Button></td><td className={cellClass}><strong>{item.name}</strong></td><td className={`${cellClass} font-mono text-xs`}>{item.code}</td><td className={cellClass}>{mixins.length ? mixins.join(' + ') : 'Object local'}</td><td className={cellClass}>{item.global ? 'すべての場所' : place?.name ?? item.initialLocationCode}</td></tr>; })}
          </tbody></table>
      ...
      <EditPane open={Boolean(object)} onOpenChange={(open) => { if (!open) setEditing(null); }} eyebrow="オブジェクト" title={object?.name ?? 'オブジェクトを編集'} description="種類、初期配置、開始時の状態を編集します。" footer={<Button onClick={() => setEditing(null)}>編集を完了</Button>}>
        {object && <div className={editorClass}>
          ...
          <section aria-label="ordered Type mixins" className="grid gap-2 rounded-xl border border-[#17151f]/12 p-3">
            ...
            {object.mixinTypeCodes.map((code, index, codes) => <div key={`${code}-${index}`} className="flex items-center gap-2 rounded-lg bg-white/70 p-2">
              <span className="min-w-0 flex-1">{value.objectTypes.find((type) => type.code === code)?.name ?? code}</span>
              <Button size="sm" variant="text" disabled={index === 0} onClick={() => { const next = [...codes]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; replaceObject({ ...object, mixinTypeCodes: next }); }}>↑</Button>
              <Button size="sm" variant="text" disabled={index === codes.length - 1} onClick={() => { const next = [...codes]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; replaceObject({ ...object, mixinTypeCodes: next }); }}>↓</Button>
              <Button size="sm" variant="text" onClick={() => { const next = codes.filter((_, itemIndex) => itemIndex !== index); replaceObject({ ...object, mixinTypeCodes: next }); }}>削除</Button>
            </div>)}
            <MyrialeSelect label="Type mixinを追加" value="" onValueChange={(code) => { const current = object.mixinTypeCodes; if (!current.includes(code)) { const next = [...current, code]; replaceObject({ ...object, mixinTypeCodes: next }); } }} options={value.objectTypes.filter((type) => !object.mixinTypeCodes.includes(type.code)).map((type) => ({ value: type.code, label: `${type.name} / ${type.code}` }))} />
          </section>
