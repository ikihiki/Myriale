          ...
          {activeStep === 'cover' && (
            <section className={wizardPanelClass} aria-label="表紙">
              ...
              <MarkdownEditor
                ...
                placeholder={'## シナリオの目的\n\nこの物語で体験することを書きます。\n\n- 主な目的\n- 重要な前提'}
              />
              <div className={wizardButtonRowClass}><Button variant="primary" size="sm" onClick={() => void saveDraft()} disabled={saving}>{saving ? '保存中…' : isEditing ? '変更を保存' : '下書き保存'}</Button><Button variant="secondary" size="sm" onClick={() => void consultAi('基本情報')} disabled={aiWorking}>AIに基本情報案を出してもらう</Button><Button variant="secondary" size="sm" onClick={adoptSummary}>採用して編集</Button></div>
            </section>
          )}
