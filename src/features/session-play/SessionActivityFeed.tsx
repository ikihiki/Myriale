...
const artifactClass = 'mx-auto mb-3 w-[min(100%,720px)] rounded-myr-card border border-myr-ink/14 bg-myr-session-artifact px-4 py-4';
...
export function NarrativeTurnItem({ turn }: { turn: NarrativeTurnApiResponse }) {
  return <article className="grid gap-2 rounded-myr-card border border-myr-ink/14 bg-myr-session-turn p-4" data-testid="narrative-turn-item" aria-label="公開済みNarrative Turn"><p className="m-0 max-w-none leading-[1.65] text-[#303644]"><span className="mr-2 inline-block rounded-full bg-myr-gold px-2 py-px align-middle text-myr-micro font-black tracking-[.1em] text-[#17151f]" aria-hidden="true">AI</span>{turn.narrative?.body ?? 'Narrativeを表示できません。'}</p></article>;
}
...
export function SessionExecutionItem({ execution, onAction, keepSucceededStatusVisible = false }: { execution: SessionExecutionApiResponse; onAction?: (id: string, action: 'retry' | 'cancel' | 'dismiss') => void; keepSucceededStatusVisible?: boolean }) {
  ...
  const actions = (execution.capabilities.canRetry || execution.capabilities.canCancel || canCancelInput) && <div className={`execution-actions flex min-h-6.5 justify-end gap-2 ${failed ? 'mt-1.25' : 'mt-1'}`}>
  ...
  return (
    <article className={`session-execution-item status-${execution.status} -mt-0.75 mr-2 mb-2 ml-auto w-[min(100%,680px)] justify-self-end border-0 bg-transparent p-0 text-right text-myr-caption leading-[1.45] ${executionStatusClass[execution.status]} ${succeeded && !keepSucceededStatusVisible ? 'execution-is-completing' : ''}`} data-testid={`execution-${execution.id}`} role={failed ? 'alert' : 'status'} aria-live={active ? 'polite' : undefined}>
      {execution.developmentDiagnostics ? <details className="m-0">
        <summary className="execution-diagnostics-summary min-h-5 cursor-pointer list-inside text-inherit select-none focus-visible:rounded focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-myr-iris">{statusLine}</summary>
        <div className="mt-1.75 ml-auto w-[min(100%,620px)] rounded-xl bg-myr-session-diagnostics px-4 py-3 text-left text-[#aaa2b3]">
          ...
          {execution.developmentDiagnostics.attempts.map((attempt) => <section key={attempt.id} className="mt-3.5">
            ...
            <dl className="my-1.75 mb-2.5 grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1 [&_dd]:m-0 [&_dd]:wrap-anywhere"><dt>Status</dt><dd>{attempt.status}</dd><dt>Provider</dt><dd>{attempt.provider ?? '—'} / {attempt.model ?? '—'}</dd><dt>Latency</dt><dd>{attempt.latencyMilliseconds == null ? '—' : `${attempt.latencyMilliseconds}ms`}</dd></dl>
...
function ExecutionPayload({ label, value }: { label: string; value: string }) {
  return <details className="mt-1.5 border-t border-[#aaa2b3]/18 pt-2"><summary className="cursor-pointer font-[750]">{label}</summary><pre className="mt-1.75 max-h-65 overflow-auto wrap-anywhere whitespace-pre-wrap rounded-lg bg-myr-session-payload p-2 text-left text-myr-caption text-[#c8c1d0]">{value}</pre></details>;
}
...
export function SessionActivityFeed({ session, onExecutionAction, onNoteReview, keepSucceededStatusVisible = false }: { session: SessionApiResponse; onExecutionAction?: (id: string, action: 'retry' | 'cancel' | 'dismiss') => void; onNoteReview?: (id: string, action: 'apply' | 'edit-apply' | 'reject' | 'snooze', request: NoteReviewRequest) => void; keepSucceededStatusVisible?: boolean }) {
  ...
  return <section className="grid max-h-[48vh] gap-3 overflow-auto pr-2" role="log" aria-label="Session activity" data-testid="session-activity-feed">
    {activity.map((item) => {
