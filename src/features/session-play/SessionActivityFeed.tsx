...
export function SessionInputItem({ text }: { text: string }) {
  return <article className="session-input-item mt-0.5 mr-2 mb-2 ml-13 w-fit max-w-[min(82%,620px)] justify-self-end rounded-[18px_18px_5px_18px] border border-[#c9bce4] bg-[#ebe5f8] px-3.75 py-2.75 text-[#2c2440] shadow-[0_7px_20px_rgba(58,43,83,.14)]" data-testid="session-input-item" aria-label="Player Input"><p className="m-0 font-bold leading-[1.55]">{text}</p></article>;
}
...
export function SessionExecutionItem({ execution, onAction, keepSucceededStatusVisible = false }: { execution: SessionExecutionApiResponse; onAction?: (id: string, action: 'retry' | 'cancel' | 'dismiss') => void; keepSucceededStatusVisible?: boolean }) {
  ...
  const actions = (execution.capabilities.canRetry || execution.capabilities.canCancel || canCancelInput) && <div className={`execution-actions flex min-h-6.5 justify-end gap-1.5 ${failed ? 'mt-1.25' : 'mt-1'}`}>
  ...
  return (
    <article className={`session-execution-item status-${execution.status} -mt-0.75 mr-2 mb-2 ml-auto w-[min(100%,680px)] justify-self-end border-0 bg-transparent p-0 text-right text-myr-caption leading-[1.45] ${executionStatusClass[execution.status]} ${succeeded && !keepSucceededStatusVisible ? 'execution-is-completing' : ''}`} data-testid={`execution-${execution.id}`} role={failed ? 'alert' : 'status'} aria-live={active ? 'polite' : undefined}>
      {execution.developmentDiagnostics ? <details className="m-0">
        ...
        <div className="mt-1.75 ml-auto w-[min(100%,620px)] rounded-xl bg-white/[.045] px-3.5 py-3 text-left text-[#aaa2b3]">
...
function ExecutionPayload({ label, value }: { label: string; value: string }) {
  return <details className="mt-1.5 border-t border-[#aaa2b3]/18 pt-1.5"><summary className="cursor-pointer font-[750]">{label}</summary><pre className="mt-1.75 max-h-65 overflow-auto wrap-anywhere whitespace-pre-wrap rounded-lg bg-black/18 p-2.25 text-left text-myr-caption text-[#c8c1d0]">{value}</pre></details>;
}
...
export function ImageArtifactItem({ mediaUrl, contentType }: { mediaUrl?: string | null; contentType: string }) {
  return <figure className={artifactClass} data-testid="image-artifact-item">{mediaUrl ? <img className="block max-h-105 w-full rounded-xl object-cover" src={mediaUrl} alt="生成された場面" loading="lazy" /> : <div className="grid min-h-myr-module-preview-min place-items-center rounded-xl bg-[linear-gradient(135deg,#d7d0f6,#b9dce0)] font-extrabold" role="img" aria-label="画像Artifactのプレビュー">画像プレビュー</div>}<figcaption>{contentType} / Narrativeとは独立した任意成果物</figcaption></figure>;
}
