...
export const executionFixture = (status: SessionExecutionApiResponse['status'], kind: SessionExecutionApiResponse['kind'] = 'scenario-turn'): SessionExecutionApiResponse => ({
  ...
  scenarioTurn: kind === 'scenario-turn' ? {
    ...
    currentLocation: { locationId: 'LOC-LIBRARY', label: '水没した書庫' },
    objects: [{ objectId: 'OBJ-DOOR', objectTypeId: 'door', label: '星座の扉', locationId: 'LOC-LIBRARY', publicState: { open: status === 'succeeded' || status === 'failed' } }],
    availableActions: [{ objectId: 'OBJ-DOOR', actionId: 'unlock', label: '銀の鍵をかざす', visibility: 'ai-choice' }],
    selectedAction: { objectId: 'OBJ-DOOR', actionId: 'unlock', objectLabel: '星座の扉', actionLabel: '銀の鍵をかざす', visibility: 'ai-choice' },
    postState: status === 'failed' || status === 'succeeded' ? { revision: 4, currentLocation: { locationId: 'LOC-LIBRARY', label: '水没した書庫' }, objects: [{ objectId: 'OBJ-DOOR', label: '星座の扉', publicState: { open: true } }], facts: ['星座の扉は開いている'] } : null,
  } : null,
