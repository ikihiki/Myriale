    ...
    <div className="mt-[18px] mb-1.5">
      ...
      <div className="grid gap-2.5">
        {providers.map((provider) => (
          <button key={provider.id} type="button" className="flex w-full cursor-pointer items-center gap-3 rounded-[14px] border border-myr-line-strong bg-myr-paper-bright px-4 py-3 font-bold text-myr-ink transition-[border-color,transform] duration-150 hover:-translate-y-px hover:border-[var(--ink)]" aria-label={`${provider.label}で${verb}`} onClick={() => onChoose?.(provider)}>
