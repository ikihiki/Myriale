...
export function ModuleUiHost({ execution, descriptor, api, onExecution, className = '' }: Props) {
  ...
  return <section className={`relative overflow-hidden rounded-myr-card border border-[#53687a] bg-[#101720] text-[#edf4f4] shadow-[0_24px_70px_rgba(24,35,50,0.2)] ${className}`} data-phase={phase}>
    <header className="flex items-center justify-between border-b border-[#415263] bg-[linear-gradient(100deg,#162331,#101720)] px-5 py-4"><div className="grid gap-0.5"><span className="font-mono text-[10px] font-semibold uppercase leading-[1.2] tracking-[0.16em] text-[#9fb1bd]">Isolated module surface</span><strong className="text-[15px]">{descriptor.package.moduleId}</strong></div><span className="rounded-full border border-[#7c6947] px-[9px] py-[5px] font-mono text-myr-caption font-bold text-[#e3bd75]">REV {execution.revision.toString().padStart(2, '0')}</span></header>
