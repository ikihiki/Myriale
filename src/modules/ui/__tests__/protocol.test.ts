...
describe('module UI protocol',()=>{
 it('accepts a current enabled action and a bounded host-random request',()=>{const value=envelope({randomValueCount:2});expect(isModuleUiInbound(value,'MEX-1')).toBe(true);expect(canDispatch(value as never,execution,false)).toBe(true)});
 it('rejects wrong protocol, execution, stale revisions, disabled actions, and busy hosts',()=>{
  ...
  expect(isModuleUiInbound(envelope({randomValueCount:65}),'MEX-1')).toBe(false);
