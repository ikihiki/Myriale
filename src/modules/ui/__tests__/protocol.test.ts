...
describe('module UI protocol',()=>{
 it('accepts a current enabled action',()=>{const value=envelope();expect(isModuleUiInbound(value,'MEX-1')).toBe(true);expect(canDispatch(value as never,execution,false)).toBe(true)});
