...
export const UM17AdminAiKeys: Story = { name: 'Admin: AI Provider管理', args: { initialView: 'admin-ai-keys' }, play: async ({ canvasElement, step }) => { const canvas = within(canvasElement); await step('Vault設定済みのRunpodを確認し、接続テストを行う', async () => { await expect(await canvas.findByRole('region', { name: 'AIキー管理' })).toBeVisible(); const runpod = await canvas.findByTestId('ai-key-row-runpod'); await expect(runpod).toHaveTextContent('Vault / 環境変数'); await expect(runpod).toHaveTextContent('使用中'); await userEvent.click(within(runpod).getByRole('button', { name: '接続テスト' })); await expect(await canvas.findByTestId('ai-key-notice')).toHaveTextContent('接続テストに成功'); }); } };
