...
export const UM17AdminAiKeys: Story = { name: 'Admin: AI Provider管理', args: { initialView: 'admin-ai-keys' }, play: async ({ canvasElement, step }) => { const canvas = within(canvasElement); await step('/adminをAppChrome付きの運用画面として表示する', async () => { await expect(await canvas.findByRole('main', { name: 'AI Provider管理' })).toBeVisible(); await expect(canvas.getByRole('navigation', { name: '主要セクション' })).toBeVisible(); await expect(canvas.getByTestId('app-url')).toHaveTextContent('/admin'); }); await step('Vault設定済みのRunpodを確認し、接続テストを行う', async () => { const runpod = await canvas.findByTestId('ai-key-row-runpod'); await expect(runpod).toHaveTextContent('Vault / 環境変数'); await expect(runpod).toHaveTextContent('使用中'); await userEvent.click(within(runpod).getByRole('button', { name: '接続テスト' })); await expect(await canvas.findByTestId('ai-key-notice')).toHaveTextContent('接続テストに成功'); }); } };
