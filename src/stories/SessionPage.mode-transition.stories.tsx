...
const meta = {
  ...
  render: () => <MyrialeApp initialUrl="/sessions/SES-PREP-1098" initialDb={createDemoDb('modeTransitionSession')} sessionPageContainer={MockSessionPageContainer} />,
  parameters: {
...
export const USM01ExplicitModeSwitch: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('バトル開始でSession Stateのmodeを保存し、通常のターン表示がバトル用操作へ切り替わる', async () => {
      ...
      await expect(canvas.getByRole('group', { name: 'バトルターン行動' })).toBeVisible();
    });
...
export const USM02ShowCurrentMode: Story = {
  name: 'US-M02: 現在の進行モードを理解できるようにしたい',
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    await step('判定中バッジと自由入力不可の理由を表示する', async () => {
      await userEvent.click(canvas.getByRole('button', { name: '判定開始' }));
...
export const USM03ReturnToDialogue: Story = {
...
export const USM04RecoverFromError: Story = {
  name: 'US-M04: エラーが起きても安全に復帰したい',
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    await userEvent.click(canvas.getByRole('button', { name: 'バトル開始' }));
    await step('処理エラー時に確定済みと未確定を明示する', async () => {
      await userEvent.click(canvas.getByRole('button', { name: '処理エラーを発生' }));
...
export const USM05ResumeAfterDisconnect: Story = {
  name: 'US-M05: 通信断・画面離脱が起きても再開したい',
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    await userEvent.click(canvas.getByRole('button', { name: '判定開始' }));
    await step('再接続時にモード状態を復元し、未完了処理を再提示する', async () => {
      await userEvent.click(canvas.getByRole('button', { name: '通信断から再接続' }));
      await expect(canvas.getByTestId('mode-badge')).toHaveTextContent('判定中');
...
export const USM06RestrictRewind: Story = {
...
export const USM07LogTransitions: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('遷移理由と開始時刻をログテーブルへ残す', async () => {
      await userEvent.click(canvas.getByRole('button', { name: 'バトル開始' }));
      await userEvent.click(canvas.getByRole('button', { name: '正常終了してAI対話へ戻る' }));
      const table = canvas.getByRole('table', { name: 'モード遷移ログ' });
...
export const USM08ShowMinimumInfoDuringForcedMode: Story = {
