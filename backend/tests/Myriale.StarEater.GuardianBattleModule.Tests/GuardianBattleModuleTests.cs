...
public sealed class GuardianBattleModuleTests
{
    ...
    public async Task MultipleRoundsRemainOneActiveStateUntilVictory()
    {
        ...
        Assert.Equal("guardian-defeated", second.Outcome?.Effects.Single().Payload.GetProperty("flagId").GetString());
        Assert.Equal(0, second.ViewState.GetProperty("enemyHp").GetInt32());
    }

    [Fact]
    public async Task HostRandomnessControlsEscapeAndClientAuthorityFieldsAreIgnored()
    {
        var initialized = await _module.InitializeAsync(new("init", Configuration, Json("{}"), []), default);
        var action = JsonSerializer.SerializeToElement(new { id="flee", playerHp=999, status="victory" });
        var result = await _module.DispatchAsync(new("flee", 0, Configuration, Json("{}"), initialized.State, action, [75, 0]), default);
        Assert.Equal(ModuleExecutionStatuses.Active, result.Status);
        Assert.Equal(18, result.ViewState.GetProperty("playerHp").GetInt32());
        var escaped = await _module.DispatchAsync(new("escape", 1, Configuration, Json("{}"), result.State, action, [25, 0]), default);
    ...
    public async Task FixedRandomValuesCanProduceDefeat()
    {
        ...
        var initialized = await _module.InitializeAsync(new("init", fragile, Json("{}"), []), default);
        var result = await _module.DispatchAsync(new("defeat", 0, fragile, Json("{}"), initialized.State, JsonSerializer.SerializeToElement(new { id="attack" }), [0, 4]), default);
    ...
    private ValueTask<ModuleTransitionResult> Dispatch(JsonElement state, long revision, string action, params uint[] random) =>
        _module.DispatchAsync(new($"request-{revision}-{action}", revision, Configuration, Json("{}"), state, JsonSerializer.SerializeToElement(new { id=action }), random), default);
