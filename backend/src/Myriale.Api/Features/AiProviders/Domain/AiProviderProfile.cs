...
public sealed class AiProviderProfile
{
    ...
    private AiProviderProfile(AiProviderProfileId id, string displayName, Uri baseUrl, string model, AiCredentialId credentialId, bool enabled, DateTimeOffset now, string systemPrompt)
    {
        Id = id; DisplayName = ValidateDisplayName(displayName); Adapter = AiAdapter.OpenAiCompatible;
        BaseUrl = NormalizeBaseUrl(baseUrl); Model = ValidateModel(model); SystemPrompt = ValidateSystemPrompt(systemPrompt); CredentialId = credentialId;
    ...
    [MaxLength(20_000)] public string SystemPrompt { get; private set; } = string.Empty;
    public AiCredentialId CredentialId { get; private set; }
    public bool Enabled { get; private set; }
    public long Revision { get; private set; }
    public DateTimeOffset UpdatedAt { get; private set; }

    public static AiProviderProfile Create(AiProviderProfileId id, string displayName, string baseUrl, string model, AiCredentialId credentialId, bool enabled, DateTimeOffset now, string systemPrompt = "") =>
        new(id, displayName, ParseBaseUrl(baseUrl), model, credentialId, enabled, now, systemPrompt);

    public void Update(string displayName, string baseUrl, string model, AiCredentialId credentialId, long expectedRevision, DateTimeOffset now, string systemPrompt = "")
    {
        ...
        Model = ValidateModel(model); SystemPrompt = ValidateSystemPrompt(systemPrompt); CredentialId = credentialId; Touch(now);
    }
    ...
    private static string ValidateSystemPrompt(string? value) => (value ?? string.Empty).Trim() is var prompt && prompt.Length <= 20_000 ? prompt : throw new ArgumentException("System prompt must be at most 20,000 characters.");
