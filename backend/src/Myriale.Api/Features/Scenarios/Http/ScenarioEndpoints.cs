...
public static class ScenarioEndpoints
{
    ...
    private static async Task<Results<Ok<ScenarioDraftResponse>, NotFound>> GetScenarioAsync(ScenarioId scenarioId, ClaimsPrincipal principal, ScenarioQueryService queries, CancellationToken ct)
    {
    ...
    private static async Task<IResult> CreateScenarioAsync(CreateScenarioRequest request, ClaimsPrincipal principal, CreateScenarioUseCase useCase, CancellationToken ct)
    {
        ...
        var result = await useCase.ExecuteAsync(new(authorId.Value, request), ct);
    ...
    private static async Task<IResult> UpdateScenarioAsync(ScenarioId scenarioId, CreateScenarioRequest request, ClaimsPrincipal principal, UpdateScenarioUseCase useCase, CancellationToken ct)
    {
        var authorId = UserId(principal); if (authorId is null) return TypedResults.Unauthorized();
        var result = await useCase.ExecuteAsync(new(scenarioId, authorId.Value, request), ct);
    ...
    private static async Task<IResult> RecommendHeroAsync(ScenarioId scenarioId, RecommendScenarioHeroRequest request, ScenarioQueryService queries, IHttpClientFactory clients, CancellationToken ct)
    {
    ...
    private static async Task<IResult> GetRuleDataAsync(ScenarioId scenarioId, ClaimsPrincipal principal, ScenarioQueryService scenarios, ScenarioDefinitionQueryService definitions, CancellationToken ct)
    {
    ...
    private static async Task<IResult> CreateRuleDataDraftAsync(ScenarioId scenarioId, ClaimsPrincipal principal,
        CreateScenarioDefinitionDraftUseCase useCase, CancellationToken ct)
    {
        var authorId = UserId(principal); if (authorId is null) return TypedResults.Unauthorized();
        var result = await useCase.ExecuteAsync(new(scenarioId, authorId.Value), ct);
    ...
    private static async Task<IResult> SaveRuleDataAsync(ScenarioId scenarioId, ScenarioRuleDataRequest request, ClaimsPrincipal principal, SaveScenarioDefinitionUseCase useCase, CancellationToken ct)
    {
        var authorId = UserId(principal); if (authorId is null) return TypedResults.Unauthorized();
        var result = await useCase.ExecuteAsync(new(scenarioId, authorId.Value, request), ct);
        return result.Outcome switch
    ...
    private static async Task<IResult> GetRuleDataReadinessAsync(ScenarioId scenarioId, ClaimsPrincipal principal, ScenarioQueryService scenarios, IScenarioDefinitionRepository repository, ScenarioDefinitionReadinessPolicy readiness, CancellationToken ct)
    {
        if (!await scenarios.IsOwnerAsync(scenarioId, UserId(principal), ct)) return TypedResults.NotFound();
    ...
    private static async Task<IResult> PublishRuleDataAsync(ScenarioId scenarioId, ClaimsPrincipal principal, PublishScenarioDefinitionUseCase useCase, CancellationToken ct)
    {
        var authorId = UserId(principal); if (authorId is null) return TypedResults.Unauthorized();
        var result = await useCase.ExecuteAsync(new(scenarioId, authorId.Value), ct);
        return result.Outcome switch
    ...
    private static async Task<IResult> DebugRuleDataAsync(ScenarioId scenarioId, ScenarioRuleDebugRequest request, ClaimsPrincipal principal, ScenarioQueryService scenarios, ScenarioRuleDebugService debug, CancellationToken ct)
    {
        if (!await scenarios.IsOwnerAsync(scenarioId, UserId(principal), ct)) return TypedResults.NotFound();
