...
internal sealed class FileModulePackageArtifactStore : IModulePackageArtifactStore
{
    ...
    public async Task<ModulePackageArtifactVerification> VerifyAsync(ModulePackageSnapshot package, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(exception, "Module artifact verification failed for {Digest}", package.Digest.Value);
    ...
    private async Task RequireDigestAsync(string path, ModulePackageDigest expected, CancellationToken cancellationToken)
    {
        ...
        if (actual != expected.Value) throw new InvalidDataException("Module package digest does not match catalog identity.");
    }
    ...
    private string PackagePath(ModulePackageDigest digest, ModulePackageFormat format) => Path.Combine(PackagesRoot, digest.Value + (format == ModulePackageFormat.Dll ? ".dll" : ".myriale-module"));
    private string ExpandedPath(ModulePackageDigest digest) => Path.Combine(ExpandedRoot, digest.Value);
