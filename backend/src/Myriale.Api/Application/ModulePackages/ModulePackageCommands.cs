...
namespace Myriale.Api.Application.ModulePackages;
    ...
    InstallModulePackageCommand install)
{
    public async Task<ModulePackageScanResult> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var package in packages)
        {
            ...
            catch (ModulePackageConcurrencyException) { issues.Add(new(package.Digest.Value, "モジュールパッケージが同時に更新されました。再実行してください。")); }
        }
