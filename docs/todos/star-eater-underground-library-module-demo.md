...
- [x] SDKを使った実Module packageとして実装する。
- [x] プレイヤーと敵の初期能力、利用可能スキル、勝敗条件をconfigurationで受け取る。
- [x] `攻撃`、`防御`、`スキル`、`逃走`を構造化Actionとして提供する。
- [x] HP、ダメージ、行動順、状態、ラウンド数、勝敗をModuleのprivate Stateで管理する。
- [x] public ViewStateにはプレイヤーが確認できる戦闘情報だけを含める。
- [x] ダメージなどの乱数にはホスト提供乱数だけを使用する。
- [x] 複数ラウンドを1つのModule Turn内で進行する。
- [x] Runtime UIはAction intentだけを送信し、HP、ダメージ、状態、勝敗を権威値として送信しない。
- [x] 勝利、敗北、逃走のいずれかで唯一の正式な`ModuleOutcome`を返す。
- [x] 勝利時に`guardian-defeated`を`set-flag` Effectで設定する。
- [x] AIへ公開する事実と、変更を禁止する戦闘結果を明示する。

### セッション統合

- [x] ダイス判定失敗後のNarrativeから戦闘Module Turnへ移行する。
- [x] 戦闘中の再読み込みや切断後も、同じExecutionとラウンドから再開できる。
- [x] 同一Request IDの再送でActionやダメージが二重適用されないことを確認する。
- [x] 戦闘完了後にOutcome Effectを一度だけ適用する。
- [x] 戦闘結果からNarrative Turnを自動生成する。
- [x] Narrative描写後に通常のAI対話へ復帰する。
...
- [x] ダイス失敗経路で、1つのSession内にNarrative、ダイス判定Module、Narrative、戦闘Module、Narrativeが順番に保存される。
- [x] 戦闘内部の複数操作とラウンドが、Session全体では1つのModule Turnとして保存される。
- [x] HP、ダメージ、状態、勝敗はModuleが確定し、Runtime UIやAIから改変できない。
- [x] 戦闘Module Turnを再読み込み後に継続でき、Action、Outcome、Session Effectが重複適用されない。
- [x] 戦闘完了後にNarrative handoffが自動実行され、AIが確定結果に従った描写を返す。
- [x] 強制進行終了後に通常のAI対話へ復帰できる。
- [x] 両Module packageをクリーンな開発環境へ再現可能な手順で導入できる。
- [x] 自動テスト、frontend build、Storybook build、backend buildが成功する。
