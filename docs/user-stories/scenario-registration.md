...
As a シナリオ作者
I want 作成項目を意味のまとまりごとに順番に編集したい
So that 世界設定から開始条件まで迷わず組み立てられる

期待される結果

- 作成・編集ウィザードは「基本情報、場所、人物、オブジェクト、開始状態、挿絵、動作確認」の7ステップで表示する。
- 基本情報にはタイトル、ジャンル、物語の前提、AI裁量をまとめる。
- 人物には主人公とNPCをまとめる。
- 場所とオブジェクトを別ステップにし、開始状態では開始Location、Object初期値、最初のNarrativeを設定する。
- 作成画面と編集画面で同じステップ名と順序を使う。

Storybook `play` steps

1. `step('7ステップが制作順に並ぶことを確認する', ...)`。
2. `step('人物で主人公とNPCを編集できることを確認する', ...)`。
3. `step('場所とオブジェクトを別々に編集できることを確認する', ...)`。
