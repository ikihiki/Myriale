...
Forgeへのpublish時は、生成HelmチャートにCloudNativePGの`myriale-postgres` Clusterを含めます。PR環境は1Gi、通常環境は8Giの永続ボリュームを使用します。CNPGが生成する`myriale-postgres-app` Secretの`uri`をAPIの`POSTGRES_URL`へ注入します。API起動時はPostgreSQLとSQLiteの両方でEF Coreの`EnsureCreated`を使用し、databaseが存在しない場合だけ現在のmodelからschemaを作成します。通常のプロセス再起動ではSession、Module Execution、receipt、記録済み乱数を削除しません。schema upgradeや既存データのbackfillは行わないため、model変更時は明示的なmigrationまたは開発・Preview環境のdatabase再作成が必要です。PRごとにClusterとデータが分離され、PR環境の削除時にデータベースも削除されます。
