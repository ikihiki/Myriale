...
Forgeへのpublish時は、生成HelmチャートにCloudNativePGの`myriale-postgres` Clusterを含めます。PR環境は1Gi、通常環境は8Giの永続ボリュームを使用します。CNPGが生成する`myriale-postgres-app` Secretの`uri`をAPIの`POSTGRES_URL`へ注入します。API起動時はPostgreSQLとSQLiteの両方でEF Coreの`EnsureCreated`を使用し、現在のmodelから空のdatabase schemaを作成します。schema upgradeや既存データのbackfillは行わないため、model変更時は開発・Preview環境のdatabaseを再作成します。PRごとにClusterとデータが分離され、PR環境の削除時にデータベースも削除されます。
